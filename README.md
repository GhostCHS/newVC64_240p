...
1. Run `vc64_240p.exe`
2. **Choose a WAD**
3. The panel reports both patches independently:
   ```
   240p        : CAN BE APPLIED
...
The standalone Windows build generates the Wii common key automatically on first use. No
`common-key.bin` file needs to be supplied by the user.

For running from source, Python 3.8+ and `cryptography` are required.

- A Wii with **cIOS 249** and **Priiloader** (or BootMii).

### Installing the result

Put the WAD in `SD:/wad/` and install with **YAWM ModMii Edition**. Choose **IOS249**.
Error `-1017` means that IOS has no trucha patch; try another cIOS slot.
